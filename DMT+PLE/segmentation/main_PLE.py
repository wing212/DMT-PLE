...
from utils.dataset_s import StandardSegmentationDataset
...
from utils.common_PLE import colors_city, colors_voc, categories_voc, categories_city, sizes_city, sizes_voc, \
...
def load_old_checkpoint(old_net, optimizer, lr_scheduler, is_mixed_precision, old_filename):
    base_dir = '../../../DMT+PLE/segmentation/'
    old_path = os.path.join(base_dir, old_filename)
    checkpoint = torch.load(old_path, map_location="cuda:1")
    old_net.load_state_dict(checkpoint['model'])
...
def init(batch_size_labeled, batch_size_pseudo, state, split, input_sizes, sets_id, std, mean,
    ...
    dataset = StandardSegmentationDataset
    # datasampler = DistributedSampler(dataset, num_replicas=dist.get_world_size(), rank=args.local_rank)
    if data_set == 'voc':
        base = "../../../VOC2012/"
        workers = 8
...
def train(writer, loader_c, loader_sup, validation_loader, device, criterion, net, optimizer, lr_scheduler,
    ...
    # test_pixel_accuracy1, best_mIoU = test_one_set(loader=validation_loader, device=device, net=net,
    #                                               num_classes=num_classes, categories=categories,
    #                                               output_size=input_sizes[2])
    # scaler = GradScaler()
    print("begin_best_mIoU", best_mIoU)
    ...
    while epoch < num_epochs:
        ...
        for i, data in enumerate(loader_c, 0):
            ...
            conf_mat.update(labels.flatten(), outputs.argmax(1).flatten())
                # print('dataloader time: %.2fs' % (time.time() - time_data))
                # time_loss = time.time()
            if with_sup:
            ...
            lr_scheduler.step()
            # scaler.update()
            # print('loss_check time: %.2fs' % (time.time() - time_loss))
            ...
            if current_step_num % val_num_steps == (val_num_steps - 1) or \
                ...
                #                   test_mIoU,
                #                   current_step_num)
                print('best', best_mIoU)
                print('test', test_mIoU)
                # print('loss_test time: %.2fs' % (time.time() - time_test))
                net.train()

                # if current_step_num == 1000:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch-size-labeled', type=int, default=2,
                        help='Batch size for labeled data (default: 1)')

    parser.add_argument('--batch-size-pseudo', type=int, default=14,
    ...
    parser.add_argument('--oldfilename', type=str, default=None,
                        help='The model saved from the previous iteration')
    parser.add_argument('--Increment', type=float, default=0,
                        help='parameter beta to improve pursude-label(default: 0)')
    ...
    parser.add_argument("--checkpoint-dir", type=str, default="..../DWT+PLE/segmentation",
    ...
    # torch.backends.cudnn.benchmark = False  x# Might hurt performance
            ...
            # Self-training
                ...
                if args.labeling:
                    unlabeled_loader = init(
                        ...
                        sets_id=args.sets_id, mean=mean, std=std, keep_scale=keep_scale,
                        reverse_channels=reverse_channels)
                    after_loading()
                    old_net = deeplab_v2(num_classes=num_classes)
                    load_old_checkpoint(old_net=old_net, optimizer=None, lr_scheduler=None,
                                        is_mixed_precision=args.mixed_precision, old_filename=args.oldfilename)
                    old_net.to(device)
                    time_now = time.time()
                    ratio = generate_class_balanced_pseudo_labels(net=net, device=device, loader=unlabeled_loader,
                                                                  old_net=old_net, input_size=input_sizes[2],
                                                                  label_ratio=args.label_ratio, num_classes=num_classes, Increment=args.Increment)
                    print(ratio)
